	...
	return repositories.MutationWorkItemWithReturnValue[ExpenseId]{
		Perform: func() (ExpenseId, error) {
			if err := c.addExpense(expense, expenseId); err != nil {
				c.logger.Log("%s: failed to insert err: %v", op, err)
...
func (c *postgresRepository) addExpense(expense Expense, id ExpenseId) error {
	const op = "repositories.spendings.postgresRepository.addExpense"
	c.logger.Log("%s: start[expense=%v id=%s]", op, expense, id)
	tx, err := c.db.BeginTx(context.Background(), nil)
	if err != nil {
		c.logger.Log("%s: failed to create tx err: %v", op, err)
		return err
	}
	_, err = c.db.Exec(`
INSERT INTO 
...
`, string(id), expense.Timestamp, expense.Details, int64(expense.Total), string(expense.Currency))
	if err != nil {
		c.logger.Log("%s: failed to insert expense err: %v", op, err)
		tx.Rollback()
		return err
	}
...
VALUES($1, $2, $3, $4);
		...
		if err != nil {
			c.logger.Log("%s: failed to insert share %d err: %v", op, i, err)
			tx.Rollback()
			return err
		}
	}
	if err := tx.Commit(); err != nil {
		c.logger.Log("%s: failed to commit tx err: %v", op, err)
		return err
	}
	c.logger.Log("%s: success[expense=%v id=%s]", op, expense, id)
	return nil
}
...
func (c *postgresRepository) RemoveExpense(expenseId ExpenseId) repositories.MutationWorkItem {
	...
	return repositories.MutationWorkItem{
		Perform: func() error {
			if err != nil {
				c.logger.Log("%s: failed to get expense to remove err: %v", op, err)
				return err
			}
			if expense == nil {
				c.logger.Log("%s: expense to remove not found", op)
...
func (c *postgresRepository) removeExpense(id ExpenseId) error {
	...
	if err != nil {
		c.logger.Log("%s: failed to remove expense err: %v", op, err)
		tx.Rollback()
		return err
	}
	_, err = c.db.Exec(`DELETE FROM spendings WHERE dealId = $1;`, string(id))
	if err != nil {
		c.logger.Log("%s: failed to remove shares err: %v", op, err)
		tx.Rollback()
		return err
	}
	if err := tx.Commit(); err != nil {
...
`
	...
	for rows.Next() {
		...
		err = rows.Scan(
			...
			&counterparty)
		if err != nil {
			c.logger.Log("%s: scan failed err: %v", op, err)
			return nil, err
		}
...
func (c *postgresRepository) GetExpensesBetween(counterparty1 CounterpartyId, counterparty2 CounterpartyId) ([]IdentifiableExpense, error) {
	...
	c.logger.Log("%s: start[c1=%s c2=%s]", op, counterparty1, counterparty2)
	query := `
SELECT
  s1.dealId,
  s1.counterparty,
...
`
	...
	c.logger.Log("%s: success[c1=%s c2=%s]", op, counterparty1, counterparty2)
	return expenses, nil
}
...
func (c *postgresRepository) GetBalance(counterparty CounterpartyId) ([]Balance, error) {
	...
	c.logger.Log("%s: start[counterparty=%s]", op, counterparty)
	query := `
SELECT
  d.currency,
  s2.counterparty,
...
`
	...
	if err != nil {
		c.logger.Log("%s: unexpected err %v", op, err)
		return balance, err
	}
	for _, value := range balancesMap {
		balance = append(balance, value)
	}
	c.logger.Log("%s: start[success=%s]", op, counterparty)
	return balance, nil
}
