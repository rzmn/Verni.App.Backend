...
func (c *defaultController) AddExpense(expense Expense, actor CounterpartyId) *common.CodeBasedError[AddExpenseErrorCode] {
	...
	c.logger.Log("%s: start[actor=%s]", op, actor)
	...
	if !isYourExpense {
		c.logger.Log("%s: user %s is not found in expense %v shares", op, actor, expense)
	...
	if err != nil {
		c.logger.Log("%s: cannot insert expense into db err: %v", op, err)
	...
	c.logger.Log("%s: success[actor=%s]", op, actor)
	return nil
}
...
func (c *defaultController) RemoveExpense(expenseId ExpenseId, actor CounterpartyId) (IdentifiableExpense, *common.CodeBasedError[RemoveExpenseErrorCode]) {
	...
	c.logger.Log("%s: start[expenseId=%s actor=%s]", op, expenseId, actor)
	expense, err := c.repository.GetExpense(spendings.ExpenseId(expenseId))
	if err != nil {
		c.logger.Log("%s: cannot get expense from db err: %v", op, err)
		return IdentifiableExpense{}, common.NewErrorWithDescription(RemoveExpenseErrorInternal, err.Error())
	}
	if expense == nil {
		c.logger.Log("%s: expense %s does not exists", op, expenseId)
	...
	if !isYourExpense {
		c.logger.Log("%s: user %s is not found in expense %s shares", op, actor, expenseId)
	...
	if err := transaction.Perform(); err != nil {
		c.logger.Log("%s: cannot remove expense from db err: %v", op, err)
		return IdentifiableExpense{}, common.NewErrorWithDescription(RemoveExpenseErrorInternal, err.Error())
	}
	c.logger.Log("%s: success[expenseId=%s actor=%s]", op, expenseId, actor)
...
func (c *defaultController) GetExpense(expenseId ExpenseId, actor CounterpartyId) (IdentifiableExpense, *common.CodeBasedError[GetExpenseErrorCode]) {
	...
	if expense == nil {
		c.logger.Log("%s: expense %s is not found in db", op, expenseId)
...
func (c *defaultController) GetBalance(actor CounterpartyId) ([]Balance, *common.CodeBasedError[GetBalanceErrorCode]) {
	...
	if err != nil {
		c.logger.Log("%s: cannot get balance for %s from db err: %v", op, actor, err)
