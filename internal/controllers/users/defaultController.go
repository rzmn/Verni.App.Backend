...
func (c *defaultController) Get(ids []UserId, sender UserId) ([]User, *common.CodeBasedError[GetUsersErrorCode]) {
	...
	c.logger.Log("%s: start[sender=%s, ids=%v]", op, sender, ids)
	usersFromRepository, err := c.users.GetUsers(common.Map(ids, func(id UserId) users.UserId {
		return users.UserId(id)
	}))
	if err != nil {
		c.logger.Log("%s: cannot get users from db err: %v", op, err)
	...
	if err != nil {
		c.logger.Log("%s: cannot get friend statuses from db err: %v", op, err)
		return []User{}, common.NewErrorWithDescription(GetUsersErrorInternal, err.Error())
	}
	result := []User{}
	for _, user := range usersFromRepository {
		...
		if !ok {
			c.logger.Log("%s: cannot get friend status of user %s", op, user.Id)
	...
	c.logger.Log("%s: success[sender=%s, ids=%v]", op, sender, ids)
	return result, nil
}
